...
function loadRecent(): string[] { try { return JSON.parse(localStorage.getItem(STORAGE.recentApps) || "[]"); } catch { return []; } }
function saveRecent(ids: string[]) { localStorage.setItem(STORAGE.recentApps, JSON.stringify(ids.slice(0, 4))); }
function addRecent(id: string): string[] {
  const prev = loadRecent().filter((x) => x !== id);
  const next = [id, ...prev].slice(0, 4);
  saveRecent(next);
  return next;
}
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-5 pt-2 pb-4">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-full"
            ...
            {search && <button onClick={() => setSearch("")} className="cursor-pointer bg-transparent border-none">
